...
const RarToZip = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      if (uploadedFile.name.endsWith(".rar")) {
        setFile(uploadedFile);
      } else {
        alert("Only RAR files are allowed.");
      }
    }
  };
  ...
  const handleConversion = async () => {
    ...
    try {
      ...
      alert("Conversion failed. Ensure your RAR file is valid.");
    }
  ...
  return (
    <section className="min-h-screen bg-gray-100 py-16 px-6 text-gray-900 flex flex-col items-center">
      ...
      >
        Convert your RAR archive to a ZIP file quickly and easily.
      </motion.p>
