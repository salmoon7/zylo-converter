...
const SevenZToZip = () => {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      if (uploadedFile.name.endsWith(".7z")) {
        setFile(uploadedFile);
      } else {
        alert("Only 7Z files are allowed.");
      }
    }
  };
  ...
  return (
    <section className="min-h-screen bg-gray-100 py-16 px-6 text-gray-900 flex flex-col items-center">
      ...
      >
        Convert your 7Z archive to a ZIP file quickly and easily.
      </motion.p>
      ...
      >
        <label className="cursor-pointer flex flex-col items-center space-y-2 border border-gray-300 p-4 rounded-lg hover:bg-gray-50">
          ...
          <span className="text-gray-600 text-sm">Click to Upload 7Z File</span>
          <input
            type="file"
            className="hidden"
            accept=".7z"
        ...
        >
          {loading ? "Converting..." : "Convert to ZIP"}
        </button>
